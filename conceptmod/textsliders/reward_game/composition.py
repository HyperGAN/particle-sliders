"""Fresh fixed-total-energy check with a matched reduced-style control."""
import argparse
...
from .confirmation import verify_batch
...
def freeze(home,confirmation,name):
    ...
    protocol=dict(name=name,source_confirmation=confirmation,source_protocol_sha256=digest(p),candidate=c,cases=cases,arms=arms,
        manifest_sha256=digest(m),sources={str(Path(__file__).resolve()):sha(__file__)},physical_gpu=1,new_clip_budget=16,
        original_style_energy=2.,reduced_style_energy=1.,reward_energy=1.,combined_energy=2.,
...
def score(home,name):
    ...
    summary=f'Fixed-total-energy composition: {len(rows)}/8 valid, {wins}/8 wins, mean {mean}, worst {worst}, pass {passed}.\n'
    ...
    (folder/'index.html').write_text('<!doctype html><meta charset="utf-8"><title>Composition check</title><h1>Fixed-total-energy composition</h1><p>'+html.escape(summary)+'</p>'+body)
    return result
