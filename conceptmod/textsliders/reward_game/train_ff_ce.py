"""Direct CE on truncated acoustic trajectories with a frozen latent anchor.

Only the final two flow steps of each chunk are differentiated. Earlier states,
conditioning, and chunk overlap are fixed training references. Deployment still
uses all ordinary flow steps and a genuine additive rank-8 attention adapter.
"""
import argparse
...
from .ff_artifact import STRUCTURE,checkpoint
...
def train(home,folder,game_home,total=8,lr=.001):
    import torch
    ...
    if total not in (8,16):raise IntegrityError('Acoustic checkpoints cover complete eight-example passes')
    ...
    ff_audit=Path(read(game_home/'audit/host-audits.json')['audit_folder'])
    if not read(ff_audit/'gradient-result.json')['passed'] or not read(ff_audit/'gradient-result.json')['frozen_attention_unchanged']:raise IntegrityError('Feed-forward-specific actual gradient audit is required')
    audit=Path(collection['reused_audit_folder'])
    ...
    sources=[Path(__file__),Path(__file__).with_name('acoustic_tail.py'),Path(__file__).with_name('merged_forward.py'),Path(__file__).with_name('ce_wave_gradient.py'),Path(__file__).with_name('ff_artifact.py'),Path(__file__).with_name('ff_network.py')]
    recipe=dict(method='direct-ce-truncated-acoustic-feed-forward-v1',initial='zero acoustic LoRA; fresh Kaiming down factors and zero up factors',
        structure=STRUCTURE,trainable_support='72 feed-forward projections only; 144 attention deltas frozen at exact zero in the ordinary block export',seed=9131,learning_rate=lr,optimizer='AdamW',betas=[.9,.999],weight_decay=0.,gradient_norm=1.,
        ...
        gradient_scope='Last two of thirty acoustic flow steps per chunk; earlier flow and inter-chunk overlap remain fixed reference values',
        ...
        checkpoint_steps=[8,16],collection_recipe_sha256=sha(folder/'collection-recipe.json'),captures_sha256=sha(folder/'captures.json'),
        acoustic_gradient_audit_sha256=sha(audit/'gradient-result.json'),feed_forward_gradient_audit_sha256=sha(ff_audit/'gradient-result.json'),reward_spec=game['reward_spec'],physical_gpu=1,
    ...
    with locked(folder/'train.lock'):
        ...
        network=make(tf)
        frozen_attention_before={k:v.detach().cpu().clone() for k,v in network.state_dict().items() if '-attn-' in k}
        ...
        def export(step):
            check_frozen_attention(network,frozen_attention_before)
            path=folder/f'reward-ce-acoustic-ff_step{step}.safetensors';save_file({k:v.detach().cpu().contiguous() for k,v in network.state_dict().items()},str(path))
            write(path.with_suffix('.json'),dict(STRUCTURE,weights_sha256=sha(path),prompts_file=str(folder/'training-prompts.json'),steps=step,
                reward=dict(name='reward-ce-acoustic-ff',method=recipe['method'],recipe_sha256=digest(recipe),interpretation=recipe['gradient_scope'],full_generation_gradient=False)))
        ...
        try:
            ...
            while completed<total and not stop:
                ...
                print('ACOUSTIC UPDATE',metric,flush=True);save()
                if completed in (8,16):export(completed);save_tensor(folder/f'state-step{completed}.pt',torch.load(folder/'state.pt',map_location='cpu',weights_only=True))
            save()
            ...
            exact=check_frozen_attention(network,frozen_attention_before) and wrapper.base_unchanged() and all(p.grad is None for p in tf.parameters()) and all(p.grad is None for p in vocoder.parameters())
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--home',required=True);p.add_argument('--folder',required=True);p.add_argument('--game-home',required=True);p.add_argument('--total',type=int,default=8);p.add_argument('--lr',type=float,default=.001);a=p.parse_args()
    train(a.home,a.folder,a.game_home,a.total,a.lr)
