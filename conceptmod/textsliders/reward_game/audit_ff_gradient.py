...
def audit(home,folder):
    ...
    source_paths=[Path(__file__),Path(__file__).with_name('acoustic_tail.py'),Path(__file__).with_name('ce_wave_gradient.py'),
                  Path(__file__).with_name('audit_ce_gradient_v3.py'),Path(__file__).with_name('ff_network.py'),Path(__file__).with_name('merged_forward.py')]
    protocol=dict(capture_sha256=sha(capture_file),source_hashes={str(p.resolve()):sha(p) for p in source_paths},
        ...
        target_replace=['MiniMaxMusic3TransformerBlock'],prefix='lora_unet',delimiter='-',seed=9131,trainable_support='72 feed-forward projections; 144 attention up factors frozen at exact zero',
    ...
    with locked(folder/'gradient.lock'),gpu_lease(home,1):
        ...
        try:
            ...
            frozen_attention=check_frozen_attention(network,before)
            ...
            wrapper.detach()
            comp=dict(kind='transformer',rank=8,alpha=8,target_replace=['MiniMaxMusic3TransformerBlock'],prefix='lora_unet',delimiter='-',train_method='full',
            ...
            result=dict(passed=changed and merged_exact and base_frozen and frozen_attention,frozen_attention_unchanged=frozen_attention,zero_waveform_exact=exact,gradient_wave_max_error=wave_max_error,
            ...
            immutable(folder/'gradient-result.json',result);Store(home).event('acoustic_ff_gradient_audit',passed=result['passed'],discarded_optimizer_updates=updates)
