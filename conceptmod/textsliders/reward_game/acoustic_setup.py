...
from .acoustic_artifact import STRUCTURE
...
def init(home,parent,audit_folder):
    ...
    amended=deepcopy(game);amended.update(game_id=game['game_id']+'-acoustic-v1',parent_benchmark_sha256=digest(game),
        candidate_structure=STRUCTURE,host_energy_by_kind=limits,
        experiment='Separate acoustic attention adapter host. Same development cases/order/thresholds/controls and frozen scoring. Host budgets are separate.')
    ...
    paths=[Path(__file__).with_name(n) for n in ('acoustic_setup.py','acoustic_worker.py','acoustic_evaluate.py','acoustic_identity.py','acoustic_renderer.py','acoustic_artifact.py')]
    immutable(home/'renderer.json',dict(version='acoustic-v1',sources={str(p.resolve()):sha(p) for p in paths},
