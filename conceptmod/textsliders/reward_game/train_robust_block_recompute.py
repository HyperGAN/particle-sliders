...
def train(home, folder):
    ...
    with locked(folder/'train.lock'):
        ...
        recomputed = RecomputedFlowForward(tf, max_rss_gib=28, max_swap_gib=2)
            ...
            while completed < 2 and not stopped:
                ...
                for row in rows[len(pending):]:
                    ...
                    for index in range(len(capture['chunks'])):
                        ...
                        print('ROBUST BLOCK CHUNK', completed+1, row['id'], index+1, recomputed.metrics, flush=True)
                    ...
                    metric = dict(case_id=row['id'], family=case['family']['family'], seed=case['seed'], ce=ce,
                        ...
                        accumulated_group_gradient_absolute_sum={k: sum(v) for k, v in groups.items()}, graph_storage_metrics=dict(recomputed.metrics), seconds=time.monotonic()-began)
                ...
                recomputed.detach(); wrapper.detach(); generator._merge_sliders(pipe, 'cuda:0', [component(path)])
                with torch.no_grad():
