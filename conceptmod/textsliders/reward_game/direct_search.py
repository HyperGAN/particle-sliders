"""Direct CE selection over predeclared depth gains on training families only."""
import argparse
...
from .gains import fold, training_objective


def choose(rows):
    eligible=[r for r in rows if r['valid'] and r['objective']>0]
    return max(eligible,key=lambda r:(r['objective'],-sum((g-.5)**2 for g in r['gains']))) if eligible else None
...
def run(home,folder,proposal_path,runner=dispatch):
    ...
    with locked(folder/'method.lock'):
        ...
        jobs=[];candidates=[]
        for index,gains in enumerate(proposal['changed_variables']['depth_gains']):
            ...
            if not path.exists():
                ...
                sidecar.update(plus_label='CE',minus_label='Off',unipolar=True,recommended_range=[0.,1.],
                               ...
                               reward=dict(method='direct-depth-gain-search-v1',parent_sha256=base['weights_sha256'],
                                           depth_gains=gains,local_optimizer_updates=0,proposal_sha256=digest(proposal),
            ...
            if any(not torch.equal(exported[k],v) for k,v in expected.items()):raise IntegrityError('Exported depth gains differ from declaration')
        ...
        if pending:
            store.event('training_selection_started',method='direct-depth-gain-search-v1',folder=str(folder),scheduled=len(pending))
            runner(home,pending)
        ...
        result=dict(method='direct-depth-gain-search-v1',actual_updates=0,results=results,
