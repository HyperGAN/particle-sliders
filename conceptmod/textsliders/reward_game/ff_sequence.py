"""Audit the separate Off bridge, then train and evaluate feed-forward factors."""
...
def run(home):
    ...
    for stage,argv in (
        ('feed_forward_off_audit',[sys.executable,'-u','-m','conceptmod.textsliders.reward_game.ff','--home',str(home),'audit-off']),
        ('feed_forward_training_and_development',[sys.executable,'-u','-m','conceptmod.textsliders.reward_game.ff_search','--home',str(home),'search','--resume'])):
