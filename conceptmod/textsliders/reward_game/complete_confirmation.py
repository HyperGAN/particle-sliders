"""Package a fixed ordinary LM adapter only after every declared confirmation check."""
import argparse
from pathlib import Path
import shutil
import json
from .core import read,write,immutable,sha,checkpoint,IntegrityError,Store
from .confirmation import verify_batch,score
from .composition import score as composition_score
...
def complete(home,first,replication,composition,name):
    ...
    result=dict(research_complete=True,scope='Frozen first-20-second CE with independently measured preservation and diversity; natural full-song completion remains unestablished',
        ...
        evidence_sha256={str(p):sha(p) for p in evidence},cost=report(home),production_deployment=False)
    immutable(folder/'evidence.json',result)
    (folder/'README.md').write_text('Confirmed ordinary reward adapter at multiplier '+str(c['multiplier'])+'.\n\nBoth fresh batches independently passed the fixed practical CE gates and whole-family uncertainty intervals, plus the declared preservation diagnostics. The separate fixed-total-energy composition comparison passed. Exact evidence, raw audio, losses and listening players are linked in this directory.\n\nThe supported scope is the frozen first 20 seconds of generated music. Natural full-song completion remains unestablished. This research package has not been added to the production registry.\n')
