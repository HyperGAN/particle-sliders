"""Separately frozen latent-target attention fresh confirmation. A passing batch still requires replication."""
import argparse
...
from .transport_confirmation_support import component
...
def freeze(home, evaluation, fixtures, name, replication_of=None):
    ...
    from .transport_confirmation_support import check_fresh
    freshness=check_fresh(home,base,game,families,seeds)
    template=read(parent/'recipes/confirmation-template-latent-transport-v1.json')
    if replication_of:
    ...
    source_names=('transport_confirmation.py','transport_confirmation_support.py','confirmation_amendments.py','acoustic_artifact.py','joint_confirmation_support.py','acoustic_renderer.py','confirmation_stats.py','transport_composition.py','transport_complete_confirmation.py')
    sources={str(Path(__file__).with_name(n).resolve()):sha(Path(__file__).with_name(n)) for n in source_names}
    diagnostic_files=[Path(__file__).with_name(n) for n in ('transport_confirmation_intent.py','transport_intent_locked.py')] + [Path(__file__).resolve().parents[3]/'slider_selection/features.py',Path(__file__).resolve().parents[3]/'scripts/lm_score.py',Path(__file__).parent.parent/'reward_sliders/diagnostics.py',
    ...
    protocol=dict(version='latent-transport-v1',name=name,evaluation=evaluation,candidate=candidate,original=original,fixtures_sha256=sha(fixtures),
