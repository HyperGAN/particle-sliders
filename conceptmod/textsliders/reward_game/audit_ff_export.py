"""Check the actual eight-update FF state and ordinary exported tensors on CPU."""
import argparse
...
def audit(home,folder):
    ...
    if status['state']!='checkpoint_ready' or status['actual_updates']!=8:
        raise IntegrityError('Expected the completed eight-update FF export')
    ...
    attention=[k for k in initial if '-attn-' in k]
    if not all(torch.equal(initial[k],exported[k]) for k in attention):
        raise IntegrityError('Frozen attention differs from the actual seeded initialization')
    ...
    if len(selected)!=144 or optimizer.keys()!=selected.keys():raise IntegrityError('Optimizer state includes frozen factors or omits FF factors')
    ...
    if not changed:raise IntegrityError('No feed-forward factor changed')
    result=dict(passed=True,checkpoint=candidate,training_state_sha256=sha(state_path),actual_updates=8,
        tensor_count=len(exported),export_matches_state_exactly=True,seeded_attention_tensors_unchanged=len(attention),
        feed_forward_optimizer_states=len(optimizer),all_optimizer_steps=8,changed_feed_forward_factors=len(changed),
    ...
    immutable(home/'audit/acoustic-ff-export-v1.json',result)
    return result
