...
def audit(home=DEFAULT_HOME):
    ...
    home=Path(home);game,_=verify(home);folder=home/'audit/ce-input-gradient-v3'
    ...
    immutable(folder/'protocol.json',dict(audio_sha256=row['audio_sha256'],physical_gpu=None,new_rendered_clips=0,
        ...
        finite_difference_epsilons=[1e-5,1e-6,1e-7,3e-8,1e-8,3e-9],finite_difference_relative_tolerance=.15,
        success_rule='All three smallest perturbations must agree with the analytical derivative within the original 15 percent tolerance'))
    ...
    for epsilon in (1e-5,1e-6,1e-7,3e-8,1e-8,3e-9):
    ...
    converged=all(c['relative_error']<.15 for c in checks[-3:])
