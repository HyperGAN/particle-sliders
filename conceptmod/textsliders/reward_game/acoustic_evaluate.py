...
from .core import (Store, read, write, immutable, digest, sha, locked,
                   candidate_identity, scorecard, markdown, IntegrityError, ROOT)
from .setup import verify
from .acoustic_artifact import checkpoint
from .acoustic_identity import generation_identity
    ...
    def batch(gpu):
        ...
        with gpu_lease(home,gpu):
            with (folder/(key+f'-gpu{gpu}.log')).open('a') as log:
                process=subprocess.Popen([sys.executable,'-u','-m','conceptmod.textsliders.reward_game.acoustic_worker',
