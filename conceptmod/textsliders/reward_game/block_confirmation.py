...
from .block_confirmation_support import component
...
def freeze(home, evaluation, fixtures, name, replication_of=None):
    ...
    from .block_confirmation_support import check_fresh
    freshness=check_fresh(home,base,game,families,seeds)
    template=read(parent/'recipes/confirmation-template-block-v1.json')
    if replication_of:
    ...
    source_names=('block_confirmation.py','block_confirmation_support.py','confirmation_amendments.py','block_artifact.py','joint_confirmation_support.py','block_renderer.py','confirmation_stats.py','block_composition.py','block_complete_confirmation.py')
    sources={str(Path(__file__).with_name(n).resolve()):sha(Path(__file__).with_name(n)) for n in source_names}
    diagnostic_files=[Path(__file__).with_name(n) for n in ('block_confirmation_intent.py','block_intent_locked.py')] + [Path(__file__).resolve().parents[3]/'slider_selection/features.py',Path(__file__).resolve().parents[3]/'scripts/lm_score.py',Path(__file__).parent.parent/'reward_sliders/diagnostics.py',
    ...
    protocol=dict(version='block-v1',name=name,evaluation=evaluation,candidate=candidate,original=original,fixtures_sha256=sha(fixtures),
