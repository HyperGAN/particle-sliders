...
from .joint_identity import generation_identity


def run(home, jobs_path, gpu):
    import torch
    from .joint_renderer import JointRenderer as SearchRenderer
    from ..reward_sliders.reward import CEReward
    from ..reward_sliders.specs import RewardSpec
    from .joint_artifact import component
        ...
        for job in jobs:
            ...
            if job.get('engineering_audit'):
                ...
                with locked(folder/'claim.lock'):
                    ...
                    result=dict(passed=False,pcm_equal=equal,physical_gpu=gpu,case=case['id'],timing=timing,
                    ...
                    if equal:
                        from .joint_merge_audit import audit
                        merged=audit(renderer,game['audit_candidate']['path'],folder)
                        result.update(passed=merged['passed'],joint_merge_audit_sha256=sha(folder/'joint-merge.json'))
                        write(result_path,result)
                    store.event('off_pcm_audit',passed=result['passed'],new_clips=1)
                    if not equal: raise IntegrityError('Off decoded PCM differs from frozen control')
                    if not result['passed']:raise IntegrityError('Joint ordinary merge audit failed')
                continue
