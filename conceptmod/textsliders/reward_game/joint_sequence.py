"""Audit the fixed pair before its registered development attempt."""
...
def run(home):
    ...
    for stage,argv in (
        ('joint_actual_host_audit',[sys.executable,'-u','-m','conceptmod.textsliders.reward_game.joint','--home',str(home),'audit-off']),
        ('joint_development_search',[sys.executable,'-u','-m','conceptmod.textsliders.reward_game.joint_search','--home',str(home),'search','--resume'])):
