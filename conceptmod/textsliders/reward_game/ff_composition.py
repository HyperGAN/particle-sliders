...
from .ff_confirmation import verify_batch
from .ff_artifact import checkpoint,component
...
def freeze(home,confirmation,name):
    ...
    protocol=dict(name=name,source_confirmation=confirmation,source_protocol_sha256=digest(p),candidate=c,cases=cases,arms=arms,
        manifest_sha256=digest(m),sources={str(Path(__file__).with_name(n).resolve()):sha(Path(__file__).with_name(n)) for n in ('ff_composition.py','ff_artifact.py','ff_renderer.py')},physical_gpu=1,new_clip_budget=16,
        version='ff-per-host-v1',
