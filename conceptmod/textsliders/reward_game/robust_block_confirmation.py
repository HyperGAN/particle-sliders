"""Separately frozen acoustic attention plus feed-forward fresh confirmation. A passing batch still requires replication."""
import argparse
...
from .robust_block_confirmation_support import component
...
def freeze(home, evaluation, fixtures, name, replication_of=None):
    from .block_evaluate import inspect
    ...
    from .robust_block_confirmation_support import check_fresh
    freshness=check_fresh(home,base,game,families,seeds)
    template=read(parent/'recipes/confirmation-template-robust-block-v1.json')
    if replication_of:
    ...
    source_names=('robust_block_confirmation.py','robust_block_confirmation_support.py','confirmation_amendments.py','block_artifact.py','joint_confirmation_support.py','block_renderer.py','confirmation_stats.py','robust_block_composition.py','robust_block_complete_confirmation.py')
    sources={str(Path(__file__).with_name(n).resolve()):sha(Path(__file__).with_name(n)) for n in source_names}
    diagnostic_files=[Path(__file__).with_name(n) for n in ('robust_block_confirmation_intent.py','robust_block_intent_locked.py')] + [Path(__file__).resolve().parents[3]/'slider_selection/features.py',Path(__file__).resolve().parents[3]/'scripts/lm_score.py',Path(__file__).parent.parent/'reward_sliders/diagnostics.py',
    ...
    model_lock=parent/'audit/confirmation-intent-cache-v1.json'
    lock=read(model_lock)
    if not lock['passed'] or not lock['actual_processors_and_models_loaded']:
        raise IntegrityError('Actual independent diagnostic model preflight is required')
    diagnostics['model_lock_sha256']=sha(model_lock)
    protocol=dict(version='robust-block-v1',name=name,evaluation=evaluation,candidate=candidate,original=original,fixtures_sha256=sha(fixtures),
        cases=cases,seeds=seeds,physical_gpu=1,arms=arms,template=template,freshness=freshness,replication_of=replication_of,
    ...
    immutable(folder/'intent/model-lock.json',lock)
    if sha(folder/'intent/model-lock.json')!=diagnostics['model_lock_sha256']:
        raise IntegrityError('Diagnostic lock copy differs from preflight')
...
def verify_batch(home, folder):
    ...
    if sha(folder/'intent/model-lock.json')!=p['diagnostics']['model_lock_sha256']:
        raise IntegrityError('Frozen diagnostic model lock changed')
                ...
                for arm in p['arms']:
                    ...
                    if file.exists():
                        ...
                        if old['status']=='running':
                            ...
                            write(file,old)
                        continue
                    if renderer is None:renderer=AcousticBlockRenderer(folder,m,1)
