"""Positive semantic-plus-residual imitation with deployment-matched LoRA merging.
...
def train(folder,gpu,total,lr=1e-6):
    ...
    with locked(folder/'train.lock'):
        ...
        initial=dict(manifest['preference']['initial'],name='original-half');seed=71
        ...
        recipe=dict(method='positive-semantic-residual-imitation-merged-v1',initial=initial,learning_rate=lr,
        ...
        def export(step):
            path=folder/f'reward-ce-residual-imitation_step{step}.safetensors'
            ...
            metadata.update(steps=step,prompts_file=str(source/'manifest.json'),weights_sha256=sha(path),
                reward=dict(name='reward-ce-residual-imitation',method=recipe['method'],recipe_sha256=digest(recipe),
