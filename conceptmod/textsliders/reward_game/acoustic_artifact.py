"""Separate, explicitly typed rank-8 acoustic attention adapter format.
...
STRUCTURE=dict(kind='transformer',rank=8,alpha=8,target_replace=['MiniMaxMusic3Attention'],
...
def checkpoint(path,multiplier=1.):
    ...
    for layer in range(36):
        for projection in ('to_q','to_k','to_v','to_out-0'):
            name=f'lora_unet-transformer_blocks-{layer}-attn-{projection}'
            keys=[name+s for s in ('.lora_up.weight','.lora_down.weight','.alpha')];expected.update(keys)
            if not set(keys)<=set(values):raise IntegrityError('Missing acoustic attention tensors')
            up,down,alpha=[values[k] for k in keys]
            if up.shape!=(2048,8) or down.shape!=(8,2048) or alpha.numel()!=1 or float(alpha)!=8:
                raise IntegrityError('Wrong acoustic projection dimensions or alpha')
