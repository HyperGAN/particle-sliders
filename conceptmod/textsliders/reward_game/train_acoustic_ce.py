...
def train(home,folder,game_home,total=8,lr=.001):
    ...
    sources=[Path(__file__),Path(__file__).with_name('acoustic_tail.py'),Path(__file__).with_name('merged_forward.py'),Path(__file__).with_name('ce_wave_gradient.py'),Path(__file__).with_name('acoustic_artifact.py')]
    recipe=dict(method='direct-ce-truncated-acoustic-attention-v1',initial='zero acoustic LoRA; fresh Kaiming down factors and zero up factors',
        ...
        acoustic_gradient_audit_sha256=sha(audit/'gradient-result.json'),reward_spec=game['reward_spec'],physical_gpu=1,
    ...
    with locked(folder/'train.lock'):
        ...
        def export(step):
            path=folder/f'reward-ce-acoustic_step{step}.safetensors';save_file({k:v.detach().cpu().contiguous() for k,v in network.state_dict().items()},str(path))
            write(path.with_suffix('.json'),dict(STRUCTURE,weights_sha256=sha(path),prompts_file=str(folder/'training-prompts.json'),steps=step,
                reward=dict(name='reward-ce-acoustic',method=recipe['method'],recipe_sha256=digest(recipe),interpretation=recipe['gradient_scope'],full_generation_gradient=False)))
