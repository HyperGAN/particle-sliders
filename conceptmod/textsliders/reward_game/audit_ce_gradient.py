...
def audit(home=DEFAULT_HOME):
    ...
    home=Path(home);game,_=verify(home);folder=home/'audit/ce-input-gradient-v1'
    ...
    immutable(folder/'protocol.json',dict(audio_sha256=row['audio_sha256'],physical_gpu=None,new_rendered_clips=0,
        ...
        finite_difference_epsilon=1e-5,finite_difference_relative_tolerance=.15))
    ...
    predicted=float((gradient*direction).sum());epsilon=1e-5
    ...
    passed=normalization_exact and scalar_error<1e-5 and abs(normal-float(differentiable.detach()))<1e-5 and relative<.15 and exact_restoration
    result=dict(passed=passed,normalization_exact=normalization_exact,cached_ce=row['reward']['scalar'],
        ...
        measured_directional_derivative=measured,finite_difference_relative_error=relative,
