"""Declared legacy-forward direct CE through all thirty flow steps within each chunk.
...
def train(home,folder,game_home,full_audit_folder,previous_audit_folder,total=8,lr=.001):
    import torch
    ...
    if full_protocol['collection_recipe_sha256']!=sha(folder/'collection-recipe.json') or full_protocol['previous_audit_result_sha256']!=sha(previous_audit_folder/'gradient-result.json'):
    ...
    sources=[Path(__file__),Path(__file__).with_name('acoustic_tail.py'),Path(__file__).with_name('merged_forward.py'),Path(__file__).with_name('full_flow.py'),Path(__file__).with_name('ce_wave_gradient.py'),Path(__file__).with_name('acoustic_artifact.py')]
    recipe=dict(method='direct-ce-full-within-chunk-flow-attention-legacy-v1',initial='zero acoustic LoRA; fresh Kaiming down factors and zero up factors',
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--home',required=True);p.add_argument('--folder',required=True);p.add_argument('--game-home',required=True);p.add_argument('--full-audit-folder',required=True);p.add_argument('--previous-audit-folder',required=True);p.add_argument('--total',type=int,default=8);p.add_argument('--lr',type=float,default=.001);a=p.parse_args()
    train(a.home,a.folder,a.game_home,a.full_audit_folder,a.previous_audit_folder,a.total,a.lr)
