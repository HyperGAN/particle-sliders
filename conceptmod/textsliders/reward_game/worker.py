...
from .core import Store, read, write, sha, digest, checkpoint, generation_identity, IntegrityError, locked
...
def run(home, jobs_path, gpu):
    ...
    store=Store(home);game,manifest=verify(home);jobs=read(jobs_path);renderer=None
