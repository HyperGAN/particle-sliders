...
def main():
    ...
    if a.action=='register':result=ff_controller.register(home,a.recipe)
    else:
        Store(parent).status('acoustic_candidate_search',research_complete=False,game_home=str(home))
        result=ff_controller.search(home)
        Store(parent).status('ff_search_review_required',research_complete=False,game_home=str(home),result=result)
