...
def prepare(home):
    ...
    folder = home/'training/acoustic-full-flow-recompute-v1'
    audit = home/'audit/acoustic-full-flow-recompute-v1'
    collection_folder = home/'training/acoustic-full-flow-legacy-v1'
    previous = home/'audit/acoustic-tail-v1'
    game_home = home/'acoustic-v1'
    intent_path = home/'recipes/full-within-chunk-flow-recompute-intent-v1.json'
    ...
    collection = read(collection_folder/'collection-recipe.json')
    captures = read(collection_folder/'captures.json')
    ...
    # The existing acoustic archive interface expects these metadata files in
    # the training folder. Link the original evidence; generate no new clips.
    folder.mkdir(parents=True, exist_ok=True)
    for name in ('collection-recipe.json', 'captures.json', 'collection-status.json',
                 'collection-off-restoration.json'):
        source = collection_folder/name
        link = folder/name
        if not link.exists():
            link.symlink_to(source)
        if link.resolve() != source or sha(link) != sha(source):
            raise IntegrityError('Reused collection metadata differs')
    immutable(folder/'reused-collection.json', dict(source=str(collection_folder),
        exact_captures_reused=8, new_audio_generated=0))
    ...
    recipe['budget'].update(optimizer_updates=8, new_training_clips=0,
        new_duplicate_engineering_clips=0, reused_full_flow_captures=8,
        new_clips=16, max_seconds=14400)
    recipe['argv'] = [recipe['argv'][0], '-u', '-m',
        'conceptmod.textsliders.reward_game.train_full_flow_recompute_ce', '--home', str(home),
        '--game-home', str(game_home), '--folder', str(folder),
        '--full-audit-folder', str(audit), '--previous-audit-folder', str(previous),
        '--collection-folder', str(collection_folder),
        '--total', '8', '--lr', '0.001']
    recipe['preprocessing'].update(
        acoustic_collection=str(collection_folder/'collection-recipe.json'),
        collection_recipe_sha256=sha(collection_folder/'collection-recipe.json'),
        captures_sha256=sha(collection_folder/'captures.json'),
        ...
        independent_training_clips_added=0, duplicate_engineering_clips_added=0,
        prior_duplicate_engineering_captures_reused=8)
    path = home/'recipes/acoustic-full-flow-recompute-8.json'
