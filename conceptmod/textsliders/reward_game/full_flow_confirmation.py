"""Separately frozen complete within-chunk flow attention fresh confirmation. A passing batch still requires replication."""
import argparse
...
from .full_flow_confirmation_support import component
...
def freeze(home, evaluation, fixtures, name, replication_of=None):
    ...
    from .full_flow_confirmation_support import check_fresh
    freshness=check_fresh(home,base,game,families,seeds)
    template=read(parent/'recipes/confirmation-template-full-flow-v1.json')
    if replication_of:
    ...
    source_names=('full_flow_confirmation.py','full_flow_confirmation_support.py','confirmation_amendments.py','acoustic_artifact.py','joint_confirmation_support.py','acoustic_renderer.py','confirmation_stats.py','full_flow_composition.py','full_flow_complete_confirmation.py')
    sources={str(Path(__file__).with_name(n).resolve()):sha(Path(__file__).with_name(n)) for n in source_names}
    diagnostic_files=[Path(__file__).with_name(n) for n in ('full_flow_confirmation_intent.py','full_flow_intent_locked.py')] + [Path(__file__).resolve().parents[3]/'slider_selection/features.py',Path(__file__).resolve().parents[3]/'scripts/lm_score.py',Path(__file__).parent.parent/'reward_sliders/diagnostics.py',
    ...
    protocol=dict(version='full-flow-v1',name=name,evaluation=evaluation,candidate=candidate,original=original,fixtures_sha256=sha(fixtures),
