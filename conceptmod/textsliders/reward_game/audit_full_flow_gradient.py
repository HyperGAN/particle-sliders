"""Unselected complete within-chunk flow gradient audit; one discarded update."""
import argparse
...
def audit(home,folder,collection_folder,cache_audit_folder):
    import torch
    import numpy as np
    ...
    collection_folder=Path(collection_folder).resolve();cache_audit_folder=Path(cache_audit_folder).resolve()
    ...
    if not cache_result['passed'] or cache_protocol['kind']!='attention' or cache_result['protocol_sha256']!=digest(cache_protocol):
        raise IntegrityError('Actual attention cache equivalence must pass first')
    for p,h in cache_protocol['sources'].items():
        if sha(p)!=h:raise IntegrityError('Audited cache source changed')
    ...
    source_paths=[Path(__file__),Path(__file__).with_name('acoustic_tail.py'),Path(__file__).with_name('ce_wave_gradient.py'),
                  Path(__file__).with_name('audit_ce_gradient_v3.py'),Path(__file__).with_name('merged_forward.py'),Path(__file__).with_name('cached_merged_forward.py'),Path(__file__).with_name('full_flow.py')]
    protocol=dict(capture_sha256=sha(capture_file),cache_audit_result_sha256=sha(cache_audit_folder/'result.json'),collection_recipe_sha256=sha(collection_folder/'collection-recipe.json'),capture_record=row,source_hashes={str(p.resolve()):sha(p) for p in source_paths},
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--home',required=True);p.add_argument('--folder',required=True);p.add_argument('--collection-folder',required=True);p.add_argument('--cache-audit-folder',required=True);a=p.parse_args()
    print(__import__('json').dumps(audit(a.home,a.folder,a.collection_folder,a.cache_audit_folder),indent=2),flush=True)
