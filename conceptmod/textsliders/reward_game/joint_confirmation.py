"""Separately frozen fixed-pair fresh confirmation. A passing batch still requires replication."""
import argparse
...
from .joint_confirmation_support import component
...
def freeze(home, evaluation, fixtures, name, replication_of=None):
    from .joint_evaluate import inspect
    ...
    template=read(parent/'recipes/confirmation-template-joint-v1.json')
    if replication_of:
    ...
    source_names=('joint_confirmation.py','joint_confirmation_support.py','confirmation_amendments.py','joint_artifact.py','acoustic_artifact.py','joint_renderer.py','confirmation_stats.py','joint_composition.py','joint_complete_confirmation.py')
    sources={str(Path(__file__).with_name(n).resolve()):sha(Path(__file__).with_name(n)) for n in source_names}
    diagnostic_files=[Path(__file__).with_name(n) for n in ('joint_confirmation_intent.py','joint_intent_locked.py')] + [Path(__file__).resolve().parents[3]/'slider_selection/features.py',Path(__file__).resolve().parents[3]/'scripts/lm_score.py',Path(__file__).parent.parent/'reward_sliders/diagnostics.py',
    ...
    protocol=dict(version='joint-v1',name=name,evaluation=evaluation,candidate=candidate,original=original,fixtures_sha256=sha(fixtures),
                ...
                for arm in p['arms']:
                    ...
                    if file.exists():
                        ...
                        if old['status']=='running':
                            ...
                            write(file,old)
                        continue
                    if renderer is None:renderer=JointRenderer(folder,m,1)
