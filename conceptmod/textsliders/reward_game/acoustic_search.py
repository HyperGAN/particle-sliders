...
from . import acoustic_controller
...
def main():
    ...
    if a.action=='register':result=acoustic_controller.register(home,a.recipe)
    else:
        Store(parent).status('acoustic_candidate_search',research_complete=False,game_home=str(home))
        result=acoustic_controller.search(home)
        Store(parent).status('acoustic_search_review_required',research_complete=False,game_home=str(home),result=result)
