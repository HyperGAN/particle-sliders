...
def prepare(home):
    ...
    folder = home/'training/acoustic-full-flow-v1'
    audit = home/'audit/acoustic-full-flow-v1'
    cache = home/'audit/attention-cache-v1'
    game_home = home/'acoustic-v1'
    intent_path = home/'recipes/full-within-chunk-flow-intent-v1.json'
    ...
    cp = read(cache/'protocol.json'); cr = read(cache/'result.json')
    if not cr['passed'] or cp['kind'] != 'attention' or cr['protocol_sha256'] != digest(cp):
        raise IntegrityError('Actual attention cache audit must pass')
    ...
    recipe['argv'] = [recipe['argv'][0], '-u', '-m',
        'conceptmod.textsliders.reward_game.train_full_flow_ce', '--home', str(home),
        '--game-home', str(game_home), '--folder', str(folder),
        '--full-audit-folder', str(audit), '--cache-audit-folder', str(cache),
        '--total', '8', '--lr', '0.001']
    recipe['preprocessing'].update(
        ...
        attention_cache_audit_sha256=sha(cache/'result.json'),
        independent_training_clips_added=0, duplicate_engineering_clips_added=8)
    path = home/'recipes/acoustic-full-flow-8.json'
