"""Declared recomputed complete within-chunk flow gradient audit; one discarded update."""
import argparse
...
def audit(home,folder,collection_folder,previous_audit_folder):
    ...
    source_paths=[Path(__file__),Path(__file__).with_name('acoustic_tail.py'),Path(__file__).with_name('ce_wave_gradient.py'),
                  Path(__file__).with_name('audit_ce_gradient_v3.py'),Path(__file__).with_name('merged_forward.py'),Path(__file__).with_name('full_flow.py'),Path(__file__).with_name('recomputed_flow_forward.py')]
    protocol=dict(capture_sha256=sha(capture_file),previous_audit_result_sha256=sha(previous_audit_folder/'gradient-result.json'),collection_recipe_sha256=sha(collection_folder/'collection-recipe.json'),capture_record=row,source_hashes={str(p.resolve()):sha(p) for p in source_paths},
        ...
        graph_storage='reentrant checkpoint per transformer call with inner block checkpointing',
    ...
    with locked(folder/'gradient.lock'),gpu_lease(home,1):
        ...
        wrapper=MergedForward(network);recomputed=RecomputedFlowForward(tf);capture=torch.load(capture_file,map_location='cpu',weights_only=True)
        ...
        try:
            ...
            for index in range(len(capture['chunks'])):
                ...
                write(folder/'gradient-progress.json',dict(completed_chunks=index+1,total_chunks=len(capture['chunks']),seconds=time.monotonic()-began,resources=recomputed.metrics))
                print('FULL FLOW GRADIENT CHUNK',index+1,'/',len(capture['chunks']),recomputed.metrics,flush=True)
                del piece,latents,reference
            if offset!=len(raw):raise IntegrityError('Acoustic chunks did not tile the waveform')
            if recomputed.metrics['recomputed_calls']!=60*len(capture['chunks']):raise IntegrityError('Missing complete transformer recomputations')
            ...
            result=dict(passed=changed and merged_exact and base_frozen and off_wave_exact,zero_waveform_exact=exact,gradient_wave_max_error=wave_max_error,
                ...
                parts=parts,graph_storage_metrics=recomputed.metrics,seconds=time.monotonic()-began,discarded_optimizer_updates=updates,new_audio_generated=0,
