"""Declared recomputed direct CE through all thirty flow steps within each chunk.
...
from .recomputed_flow_forward import RecomputedFlowForward


def train(home,folder,game_home,full_audit_folder,previous_audit_folder,collection_folder,total=8,lr=.001):
    import torch
    ...
    collection_folder=Path(collection_folder).resolve()
    collection=read(collection_folder/'collection-recipe.json');captures=read(collection_folder/'captures.json');status=read(collection_folder/'collection-status.json')
    ...
    full_audit_folder=Path(full_audit_folder).resolve();previous_audit_folder=Path(previous_audit_folder).resolve()
    ...
    if full_protocol['collection_recipe_sha256']!=sha(collection_folder/'collection-recipe.json') or full_protocol['previous_audit_result_sha256']!=sha(previous_audit_folder/'gradient-result.json'):
        raise IntegrityError('Full-flow audit belongs to different capture/ordinary-forward evidence')
    for path,h in {**full_protocol['source_hashes'],**previous_protocol['source_hashes']}.items():
        if sha(path)!=h:raise IntegrityError('Full-flow or ordinary-forward audit source changed')
    if not read(audit/'gradient-result.json')['passed'] or not read(collection_folder/'collection-off-restoration.json')['exact']:raise IntegrityError('Acoustic prerequisites incomplete')
    ...
    immutable(folder/'training-prompts.json',dict(families=list(families.values()),capture_collection_sha256=sha(collection_folder/'collection-recipe.json')))
    sources=[Path(__file__),Path(__file__).with_name('acoustic_tail.py'),Path(__file__).with_name('merged_forward.py'),Path(__file__).with_name('full_flow.py'),Path(__file__).with_name('ce_wave_gradient.py'),Path(__file__).with_name('acoustic_artifact.py'),Path(__file__).with_name('recomputed_flow_forward.py')]
    recipe=dict(method='direct-ce-full-within-chunk-flow-attention-recompute-v1',initial='zero acoustic LoRA; fresh Kaiming down factors and zero up factors',
        ...
        collection_folder=str(collection_folder),graph_storage="reentrant checkpoint per transformer call with inner block checkpointing",
        memory_budget=dict(max_process_rss_gib=28,max_process_swap_gib=2),
        ...
        checkpoint_steps=[8],collection_recipe_sha256=sha(collection_folder/'collection-recipe.json'),captures_sha256=sha(collection_folder/'captures.json'),
        previous_tail_gradient_audit_sha256=sha(audit/'gradient-result.json'),full_flow_gradient_audit_sha256=sha(full_audit_folder/'gradient-result.json'),previous_audit_sha256=sha(previous_audit_folder/'gradient-result.json'),reward_spec=game['reward_spec'],physical_gpu=1,
    ...
    with locked(folder/'train.lock'):
        ...
        wrapper=MergedForward(network);recomputed=RecomputedFlowForward(tf);optimizer=torch.optim.AdamW(network.parameters(),lr=lr,betas=(.9,.999),weight_decay=0.)
        ...
        try:
            wrapper.attach();recomputed.attach()
            ...
            while completed<total and not stop:
                ...
                for index in range(len(capture['chunks'])):
                    ...
                    print('FULL FLOW TRAIN CHUNK',completed+1,index+1,'/',len(capture['chunks']),recomputed.metrics,flush=True)
                ...
                metric=dict(step=completed,family=case['family']['family'],seed=case['seed'],ce=ce,reference_ce=case['reference_ce'],objective=-ce+penalty_value,
                    latent_penalty=penalty_value,worst_chunk_relative_latent_l2=worst_latent,gradient_forward_wave_max_error=wave_error,gradient_norm=float(grad),graph_storage_metrics=dict(recomputed.metrics),seconds=time.monotonic()-began)
            ...
            recomputed.detach();wrapper.detach();generator._merge_sliders(pipe,'cuda:0',[])
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--home',required=True);p.add_argument('--folder',required=True);p.add_argument('--game-home',required=True);p.add_argument('--full-audit-folder',required=True);p.add_argument('--previous-audit-folder',required=True);p.add_argument('--collection-folder',required=True);p.add_argument('--total',type=int,default=8);p.add_argument('--lr',type=float,default=.001);a=p.parse_args()
    train(a.home,a.folder,a.game_home,a.full_audit_folder,a.previous_audit_folder,a.collection_folder,a.total,a.lr)
