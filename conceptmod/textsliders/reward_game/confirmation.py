"""Separately frozen fresh confirmation. A passing batch still requires replication."""
import argparse
...
import time
import json
from .core import read, write, immutable, sha, digest, checkpoint, locked, IntegrityError, Store
...
def freeze(home, evaluation, fixtures, name, replication_of=None):
    ...
    for old in (home/'confirmation').glob('*/protocol.json'):
    ...
    template=read(home/'recipes/confirmation-template-v1.json')
    if replication_of:
    ...
    sources={str(Path(__file__).resolve()):sha(__file__),str(Path(__file__).with_name('confirmation_stats.py').resolve()):sha(Path(__file__).with_name('confirmation_stats.py'))}
    diagnostic_files=[Path(__file__).with_name('confirmation_intent.py'),Path(__file__).parent.parent/'reward_sliders/diagnostics.py',
    ...
    protocol=dict(version=1,name=name,evaluation=evaluation,candidate=candidate,original=original,fixtures_sha256=sha(fixtures),
...
def verify_batch(home, folder):
    ...
    for file,expected in {**p['sources'],**m['style_hashes']}.items():
        if sha(file)!=expected:raise IntegrityError('Fresh source or style changed: '+file)
    for label in ('candidate','original'):
        c=p[label]
        if checkpoint(c['path'],c['multiplier'])!=c:raise IntegrityError('Frozen checkpoint changed')
    return p,m
                ...
                for arm in p['arms']:
                    ...
                    if file.exists():
                        ...
                        if old['status']=='running':
                            ...
                            write(file,old)
                        continue
                    if renderer is None:renderer=SearchRenderer(folder,m,1)
