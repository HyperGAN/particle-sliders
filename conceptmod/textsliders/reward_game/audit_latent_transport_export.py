"""Check the actual eight-update recomputed complete-flow attention state and ordinary exported tensors on CPU."""
import argparse
...
def audit(home,folder):
    ...
    if not read(folder/'native-step1-audit.json')['passed']:raise IntegrityError('Native transport audit failed')
    if status['state']!='checkpoint_ready' or status['actual_updates']!=80:
        raise IntegrityError('Expected the completed eighty-update latent-transport attention export')
    if not read(folder/'off-restoration-80.json')['exact']:raise IntegrityError('Training Off restoration failed')
    candidate=checkpoint(status['checkpoint'],1.);exported=load_file(candidate['path'])
    state_path=folder/'state-step80.pt';state=torch.load(state_path,map_location='cpu',weights_only=True)
    updates=[json.loads(line) for line in (folder/'updates.jsonl').read_text().splitlines() if line]
    if state['completed']!=80 or state['recipe_sha256']!=digest(recipe) or state['history']!=updates or [r['step'] for r in updates]!=list(range(1,81)):
    ...
    for ident,values in optimizer.items():
        name=selected[ident]
        if float(values['step'])!=80:raise IntegrityError('Optimizer factor has the wrong step count')
    ...
    result=dict(passed=True,checkpoint=candidate,training_state_sha256=sha(state_path),actual_updates=80,
        tensor_count=len(exported),export_matches_state_exactly=True,alpha_tensors_unchanged=len(attention),
        attention_optimizer_states=len(optimizer),all_optimizer_steps=80,changed_attention_factors=len(changed),
    ...
    immutable(home/'audit/acoustic-latent-transport-export-v1.json',result)
    return result
