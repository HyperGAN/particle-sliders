"""Separately frozen acoustic feed-forward fresh confirmation. A passing batch still requires replication."""
import argparse
...
from .ff_confirmation_support import component
...
def freeze(home, evaluation, fixtures, name, replication_of=None):
    from .ff_evaluate import inspect
    ...
    from .ff_confirmation_support import check_fresh
    freshness=check_fresh(home,base,game,families,seeds)
    template=read(parent/'recipes/confirmation-template-ff-v1.json')
    if replication_of:
    ...
    source_names=('ff_confirmation.py','ff_confirmation_support.py','confirmation_amendments.py','ff_artifact.py','joint_confirmation_support.py','ff_renderer.py','confirmation_stats.py','ff_composition.py','ff_complete_confirmation.py')
    sources={str(Path(__file__).with_name(n).resolve()):sha(Path(__file__).with_name(n)) for n in source_names}
    diagnostic_files=[Path(__file__).with_name(n) for n in ('ff_confirmation_intent.py','ff_intent_locked.py')] + [Path(__file__).resolve().parents[3]/'slider_selection/features.py',Path(__file__).resolve().parents[3]/'scripts/lm_score.py',Path(__file__).parent.parent/'reward_sliders/diagnostics.py',
    ...
    protocol=dict(version='ff-v1',name=name,evaluation=evaluation,candidate=candidate,original=original,fixtures_sha256=sha(fixtures),
...
def run(home,name):
    ...
    from .ff_renderer import FeedForwardRenderer
                ...
                for arm in p['arms']:
                    ...
                    if file.exists():
                        ...
                        if old['status']=='running':
                            ...
                            write(file,old)
                        continue
                    if renderer is None:renderer=FeedForwardRenderer(folder,m,1)
