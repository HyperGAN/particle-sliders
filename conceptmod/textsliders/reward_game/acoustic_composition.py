...
from .acoustic_confirmation import verify_batch
...
def freeze(home,confirmation,name):
    ...
    protocol=dict(name=name,source_confirmation=confirmation,source_protocol_sha256=digest(p),candidate=c,cases=cases,arms=arms,
        manifest_sha256=digest(m),sources={str(Path(__file__).with_name(n).resolve()):sha(Path(__file__).with_name(n)) for n in ('acoustic_composition.py','acoustic_artifact.py','acoustic_renderer.py')},physical_gpu=1,new_clip_budget=16,
        version='acoustic-per-host-v1',
