"""Clipped semantic-plus-residual surrogate on a frozen parent rollout batch.
...
def train(folder,gpu,total,lr=1e-6):
    ...
    with locked(folder/'train.lock'):
        ...
        recipe=dict(method='clipped-semantic-residual-parent-policy-v1',initial=initial,learning_rate=lr,
                    ...
                    sampling='equal family permutation, one saved parent rollout per update',policy_clip=.2,min_family_ce_gap=.05,
        ...
        def export(step):
            path=folder/f'reward-ce-parent-policy_step{step}.safetensors'
            ...
            metadata.update(steps=step,prompts_file=str(source/'manifest.json'),weights_sha256=sha(path),
                reward=dict(name='reward-ce-parent-policy',method=recipe['method'],recipe_sha256=digest(recipe),
                    ...
                    initial=initial,full_audio_likelihood=False,new_optimizer='AdamW for signed clipped parent-rollout objective',policy_parent_sha256=sha(folder/'policy-parent.json')))
        ...
        try:
            ...
            while completed<total and not stop:
                ...
                if not torch.isfinite(loss):raise FloatingPointError('Nonfinite clipped policy objective')
                loss.backward();grad=torch.nn.utils.clip_grad_norm_(network.parameters(),1.,error_if_nonfinite=True)
                optimizer.step();completed+=1
                metric=dict(step=completed,family=pair['family'],advantage=pair['advantage'],policy_loss=float(policy_loss.detach()),loss=float(loss.detach()),selected_semantic_mean=float(mean.detach()),
