...
def generation_identity(game,manifest,case,candidate):
    ...
    return dict(schema='ordinary-music3-acoustic-block-render-v1',candidate=candidate_identity(candidate),
        case={k:case[k] for k in ('id','family','seed','physical_gpu')},styles=styles,base_hashes=manifest['model_hashes'],
        renderer_sources=manifest['source_hashes'],bridge_sha256=sha(Path(__file__).with_name('block_worker.py')),
        block_identity_sha256=sha(__file__),host_energy_by_kind=game['host_energy_by_kind'],resolved_energy=energy,
