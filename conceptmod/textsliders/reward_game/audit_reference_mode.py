...
def audit(home,gpu):
    ...
    home=Path(home);folder=home/'audit/reference-mode-v1';source=ROOT/'analysis/reward_preference_20260908'
    ...
    signature=dict(physical_gpu=gpu,optimizer_updates=0,initial=recipe['initial'],selected=[p['chosen'] for p in pairs],
        ...
        question='Does grad-enabled checkpointed training differ from a no-grad reference before any optimizer update?')
        ...
        try:
            for pair in pairs:
                ...
                def forward():
                    ...
                    return guided_log_probs(hidden[:,boundary:][:,::16],lm.lm_head.weight,
                ...
                with torch.no_grad():evaluation=forward().cpu()
                ...
                result=dict(family=pair['family'],zero_updates=True,
