...
from .joint_confirmation import verify_batch
...
def freeze(home,confirmation,name):
    ...
    for f,styles in zip(selected,original_styles):
        ...
        f['family']=f"{name}-{f['family']}";f['style_multipliers']={k:v/2. for k,v in styles.items()}
    validate_families(selected);m=deepcopy(manifest);m['families']=selected
    m['host_energy_by_kind']=dict(language_model=2.,transformer=1.)
    ...
    protocol=dict(name=name,source_confirmation=confirmation,source_protocol_sha256=digest(p),candidate=c,cases=cases,arms=arms,
        manifest_sha256=digest(m),sources={str(Path(__file__).with_name(n).resolve()):sha(Path(__file__).with_name(n)) for n in ('joint_composition.py','joint_artifact.py','acoustic_artifact.py','joint_renderer.py')},physical_gpu=1,new_clip_budget=16,
        version='joint-per-host-v1',
        host_energy_by_kind=dict(language_model=2.,transformer=1.),
        control_effective_energy=dict(language_model=1.,transformer=0.),
        candidate_effective_energy=dict(language_model=2.,transformer=1.),
        allocation='At fixed LM energy 2 and transformer energy 1, reduce LM styles from sum 2 to sum 1 and add unit reward on each host. The style-only control uses those same reduced style multipliers.',
        original_style_energy=2.,reduced_style_energy=1.,reward_energy_by_kind=dict(language_model=1.,transformer=1.),
