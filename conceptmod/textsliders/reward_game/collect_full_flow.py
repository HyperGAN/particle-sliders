"""Unselected all-thirty-step Off captures; exact duplicate-audio accounting."""
import argparse
...
def freeze(home,folder,audit_folder):
    ...
    sources=[Path(__file__),Path(__file__).with_name('full_flow.py'),Path(__file__).with_name('acoustic_tail.py')]
    recipe=dict(method='balanced-Off-full-within-chunk-flow-capture-v1',retained_flow_steps=30,cases=rows,physical_gpu=1,source_manifest_sha256=sha(game['source_manifest']),
        source_hashes={str(p.resolve()):sha(p) for p in sources},previous_tail_audit_folder=str(audit_folder),reused_case=None,
        audit_hashes={f:sha(audit_folder/f) for f in ('capture-result.json','gradient-result.json','acoustic-tail.pt')},
        budget=dict(new_engineering_clips=8,new_independent_training_clips=0,optimizer_updates=0),
    ...
    for path,expected in recipe['source_hashes'].items():
        archived=folder/'collection-sources'/expected/Path(path).name
        archived.parent.mkdir(parents=True,exist_ok=True)
        if not archived.exists():shutil.copy2(path,archived)
        if sha(archived)!=expected:raise IntegrityError('Archived full-flow collection source differs')
    Store(home).event('full_flow_training_collection_frozen',folder=str(folder),recipe_sha256=digest(recipe),new_engineering_clip_budget=8)
    return recipe
...
def run(home,folder):
    ...
    from .full_flow import FullFlowCapture,replay as replay_latents
    ...
    audit=Path(recipe['previous_tail_audit_folder'])
    ...
    with locked(folder/'collection.lock'),gpu_lease(home,1):
        try:
            for case in recipe['cases']:
                ...
                target=folder/'captures'/ident;path=target/'off.wav';capture_file=target/'full-flow.pt'
                ...
                write(record,r)
                try:
                    with FullFlowCapture(renderer.pipe) as collector:
                    ...
                    if not all(c['exact'] for c in checks):raise IntegrityError('Complete within-chunk flow replay differs')
                    r.update(status='complete',full_flow_replay_checks=checks,reference_ce=case['reference_ce'])
                ...
                results.append(r);print('FULL FLOW CAPTURE',ident,r['status'],flush=True)
                Store(home).event('full_flow_training_capture_finished',case=ident,new_engineering_clips=r['new_clips'])
