"""Real small block host: balanced gradients survive a saved case boundary."""
...
def test_joint_groups_and_partial_batch_resume_match(dtype):
    ...
    try:
        ...
        for key in results[0]['network']:
            assert torch.equal(results[0]['network'][key], results[1]['network'][key])
        for ident, state in results[0]['optimizer']['state'].items():
            for key, value in state.items():
                assert torch.equal(value, results[1]['optimizer']['state'][ident][key])
