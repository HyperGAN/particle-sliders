...
@pytest.mark.parametrize('host', ['acoustic','ff','block','full_flow'])
def test_acoustic_confirmation_requires_full_development_pass(tmp_path, monkeypatch, host):
    ...
    acoustic_evaluate=importlib.import_module('conceptmod.textsliders.reward_game.'+('acoustic' if host=='full_flow' else host)+'_evaluate')
...
def test_fresh_manifest_carries_separate_host_limits(tmp_path, monkeypatch, host):
    ...
    if host in ('ff','block','full_flow'):write(parent/'audit/confirmation-intent-cache-v1.json',dict(passed=True,actual_processors_and_models_loaded=True,models=[]))
...
def test_composition_matches_actual_studio_allocation_without_reducing_other_host(tmp_path, monkeypatch, host):
    ...
    renderer_module=importlib.import_module('conceptmod.textsliders.reward_game.'+('acoustic' if host=='full_flow' else host)+'_renderer')
    renderer_class=getattr(renderer_module, {'acoustic':'AcousticRenderer','ff':'FeedForwardRenderer','block':'AcousticBlockRenderer','full_flow':'AcousticRenderer'}[host])
