...
PAGE = ROOT / "eval/listen/fresh-seed-stress-lofi-20260910"
RUN = WORK / "lofi-fresh-stress"
PY = "/home/mikkel/anaconda3/envs/minimax-music3/bin/python"
UNIT = "music-fresh-stress-train-20260910"
...
def weight(step):
    ...
    return RUN / f"lofi-fresh-stress_step{step}.safetensors"


def folder(step, row, seed):
    if step == 1000:
        return Path(PROTOCOL["evaluation"]["reference_page"]) / f"step-1000/row-{row}" / f"{weight(step).stem}-s{seed}"
    return PAGE / f"step-{step}/row-{row}" / f"{weight(step).stem}-s{seed}"


def records():
    reference = read(ROOT / "analysis/fresh_seed_long_20260910/scores-1000.json")["records"]
    result = {1000: [r for r in reference if r["checkpoint"]["path"] == str(weight(1000))]}
...
def publish(phase):
    ...
    for row in PROTOCOL["evaluation"]["rows"]:
        for seed in PROTOCOL["evaluation"]["seeds"]:
            takes = []
            for step in [1000, *PROTOCOL["evaluation"]["steps"]]:
    ...
    summaries = []
    baseline = {case(r): r for r in measured[1000]}
    for step, group in measured.items():
        ...
        summaries.append(dict(step=step, mean_ce=statistics.mean(r["candidate"]["enjoyment"] for r in group),
            ...
            delta_vs1000=statistics.mean(differences), wins=sum(d>0 for d in differences), total=len(group),
            paired_deltas=[dict(row=case(r)[0], seed=r["seed"], ce_delta=d) for r, d in zip(group, differences)]))
    training = read(RUN / "status.json", {"status": "starting", "completed": 1000, "total": PROTOCOL["end_steps"]})
    ...
    lines = ["# Fresh-seed Lo-fi: continued listening beyond step 1000", "", f"Status: {phase}.", "",
        "All four monitoring cases receive equal weight. The reference is fresh step 1000.", "",
        "| Step | Mean CE | CE change vs 1000 | Wins | Production quality | Lyric agreement |",
        "| --- | ---: | ---: | ---: | ---: | ---: |"]
    for value in summaries:
        lines.append(f"| {value['step']} | {value['mean_ce']:.6f} | {value['delta_vs1000']:+.6f} | "
    ...
    lines.extend(["", "These reused monitoring cases do not independently confirm checkpoint selection.",
        ...
        "[Listen](http://100.90.104.57:8888/fresh-seed-stress-lofi-20260910/)", ""])
...
def audit_step(step):
    measured = records()
    references = {case(r): r for r in measured[1000]}
    checks = []
...
def main():
    ...
    for step in PROTOCOL["evaluation"]["steps"]:
        ...
        for row in PROTOCOL["evaluation"]["rows"]:
            reuse = []
            for seed in PROTOCOL["evaluation"]["seeds"]:
                source, target = folder(1000, row, seed), folder(step, row, seed)
