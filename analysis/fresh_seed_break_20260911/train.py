"""Continue the fresh-history run from step 2000 to 5000 to find where it breaks."""
...
def history_seed(update):
    if update < 2001:
        raise ValueError("This continuation starts at update 2001")
...
def main():
    ...
    assert protocol["source_state_sha256"], "queue.py records the parent hashes before launch"
    ...
    assert base.sha(Path(protocol["source_weights"])) == protocol["source_weights_sha256"]
    checks = dict(
