...
def collect(
    *,
    ...
    field2d = score_field2d(
        cfg,
        teacher=teacher,
        # B fail-closed: a leftover-gated teacher on attribute-pinned pairs
        # is a contradiction (train_lm_adv raises). Gated teachers run on
        # unpinned pairs; faithful stays pinned.
        with_attrs=(str(teacher).strip().lower() == "faithful"),
    )
