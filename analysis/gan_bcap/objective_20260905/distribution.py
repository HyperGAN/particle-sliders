...
def paired_mmd(fake,real,*,scale=1.,bandwidths=(.03,.1,.3,1.,3.)):
    """The same RBF MMD for singleton distributions at each condition.

    Distances use RMS hidden coordinates. The kernel is fixed for the entire
    run. It is bounded and can have weak gradients at very large separation;
    unlike the energy metric it is not coercive in the hidden displacement.
    """
    if fake.shape!=real.shape or fake.numel()==0:
        raise ValueError('Paired tensors must have matching nonempty shapes')
    ...
    if not bandwidths or any(not math.isfinite(b) or b<=0 for b in bandwidths):
        raise ValueError('Invalid bandwidths')
    r2=((fake-real.detach())/s).square().mean(-1)
    return sum(-2*torch.expm1(-r2/(2*b*b)).mean() for b in bandwidths)/len(bandwidths)
