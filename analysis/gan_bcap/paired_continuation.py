...
def setup():
    ...
    protocol = dict(version=1, arms=list(ARMS), checkpoints=list(STEPS), gpu=0,
        ...
        stop_rule='Stop an arm on nonfinite training; at least two consensus diversity alarms out of eight prompts; at least four near-silent clips out of 32; or at least eight severe lyric mismatches spanning at least three prompts. Severe lyric mismatch requires word precision and phrase accuracy below .35 with BOTH references at least .65 on both measures. Low lyric-sheet coverage and metric plateau alone never stop a branch.',
...
def severe_audio_alarm(details, records=()):
    ...
    prompts = {r['fixture']: r['prompt'] for r in details['clips'] if 'fixture' in r}
    mismatches = []
    for row in records:
        candidate = row['candidate']['lyric_diagnostics']
        references = [row[k]['lyric_diagnostics'] for k in ('baseline', 'positive_reference')]
        if (all(candidate[k] < .35 for k in ('precision', 'phrase_accuracy')) and
                all(ref[k] >= .65 for ref in references for k in ('precision', 'phrase_accuracy'))):
            mismatches.append(prompts[row['fixture']])
    lyric_break = len(mismatches) >= 8 and len(set(mismatches)) >= 3
    return dict(stop=collapse >= 2 or silent >= 4 or lyric_break,
        consensus_collapse_prompts=collapse, near_silent_clips=silent,
        severe_lyric_mismatch_clips=len(mismatches), severe_lyric_mismatch_prompts=len(set(mismatches)))
...
def assess(state, step, reports, *, full):
    ...
    if full:
        ...
        for arm, weight in current.items():
            ...
            examples = [r for r in records if r['checkpoint']['path'] == weight]
            alarm = severe_audio_alarm(details, examples)
            if alarm['stop']:
