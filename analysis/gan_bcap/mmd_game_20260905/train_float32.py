"""Float32 continuation of fixed-history paired MMD, with audited precision probes.
...
from .precision import directional_probe, float32_teachers
...
def main():
    ...
    computation_sources = set(sources) | {
        ...
        str((Path(__file__).parent/name).resolve()) for name in ('adaptive.py', 'train_adaptive.py', 'precision.py', 'train_float32.py')}
    nontraining_changes = []
    ...
    train = float32_teachers(lm, tokenizer, network, train, rows, device)
    ...
    write(args.out/'precision-transition.json', dict(
        ...
        input_values='cached bf16 embeddings widened exactly; identical history samples and positions',
    ...
    manifest.update(arguments=dict(configuration, source_state=str(args.source_state.resolve()),
        ...
        scale=scale, objective='paired multiscale RBF MMD with float32 teachers on frozen neutral histories',
