...
def main():
    ...
    folder = Path(__file__).resolve().parent/'rounds/round-0002'
    training = folder/'float32-candidate'
    ...
    initial = torch.load(training/'state-step720.pt', map_location='cpu', weights_only=True)
    ...
    assert moment_steps == [adam_accepts]
    ...
    result = dict(status='passed', checkpoint_state='bitwise identical',
