"""Resume fixed-kernel research state with adaptive transactional updates on GPU 1.
...
def main():
    ...
    computation_sources = set(sources) | {
        ...
        str((Path(__file__).parent/name).resolve()) for name in ('adaptive.py', 'train_adaptive.py')}
    nontraining_changes = []
    ...
    if previous.get('prepared_sha256'):
        ...
        settings = SimpleNamespace(frames=configuration['frames'], cfg_branches=True)
        train = prepare(lm, tokenizer, rows, settings, device)
        heldout = prepare(lm, tokenizer, heldout_rows, settings, device)
