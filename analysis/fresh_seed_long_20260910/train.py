"""Extend the audited fresh-history run without resetting its training state."""
...
def history_seed(update):
    if update < 661:
        raise ValueError("This continuation starts at update 661")
...
def main():
    ...
    base.write_json(run / "status.json", dict(status="loading", completed=660, total=end))
    ...
    assert engine.completed == 660 and len(history) == 60
    blob = torch.load(source, map_location="cpu", weights_only=True)
    from safetensors.torch import load_file
    expected_weights = load_file(str(source.parent / "lofi-fresh-history_step660.safetensors"))
    checks = dict(
    ...
    signature = dict(parent_signature, experiment=protocol, experiment_code_sha256=base.sha(__file__),
        continuation_parent=dict(state=str(source), sha256=base.sha(source),
            manifest_sha256=base.sha(source.parent / "manifest.json"), completed=660))
    ...
    def save():
        ...
        if engine.completed == end:
            ...
            base.write_json(run / "completion-audit.json", dict(completed=engine.completed,
                additional_updates=end-660, total_fresh_updates=len(history), unique_seeds=len({r["history"]["seed"] for r in history}),
