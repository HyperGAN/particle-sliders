# ParticleGAN gradient regularizers — vendored, body verbatim (Strategy B).
#
# SOURCE: https://github.com/255BITS/ParticleGAN/blob/master/particlegan/grad_regularizers.py
# UPSTREAM RELEASE: particlegan 0.2.0 on PyPI (this file is byte-identical to
#   that release and to master at fetch time; it defines `GradRegularizer`
#   with the `GradientPenalty` alias). Vendored 2026-09-16; only this header
#   was added, the module body below is untouched.
#
# Why vendored instead of `pip install particlegan`: the PyPI package requires
# torch>=2.6 while this repo pins torch==2.0.1 for the Music GPU stack
# (xformers 0.0.21). A pip dependency would force a torch upgrade across the
# training environment; vendoring keeps the b_cap penalty math
# ParticleGAN-faithful without touching the dependency stack. If the pins ever
# align, replace this module with
# `from particlegan.grad_regularizers import GradRegularizer` and delete this
# file — `tests/test_music_arm_b.py::test_grad_regularizer_source_pinned`
