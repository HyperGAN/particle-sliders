#!/usr/bin/env python3
"""REVIEW COPY of lm_score.py — NOT FROZEN, NOT FOR PROMOTION DECISIONS.

Adds a pronunciation-alias layer to lyric recall (see HOMOPHONES below):
singers render phonetic fragments correctly ("Ee-lan More-in Ted-ro-nye"
-> heard "Elan Moore and Ted Rognay"; "Dragonmount" -> "Dragon Mount"),
and plain bag-of-words recall marks them missed. The alias recall reports
what the gate would say if heard homophones/compounds vindicated their
sheet fragments.

Gates still read PLAIN recall (frozen thresholds untouched). Alias numbers
are reported alongside in `lyric_recall_alias` / board `recall_alias_up`
so reviewers can see how much of a U2 failure is meter artifact vs real
garble. Writes lm_scores_alias.json (never lm_scores.json) and shares the
frozen measurement cache read-only.

Library evidence: /tmp/opencode/pron_audit.jsonl (1004 songs, whisper-large-v3-turbo).

Original docstring follows; "frozen" claims below do NOT apply to this copy.
...
# heard-word -> sheet fragment(s) it vindicates. Built from library-audit
# transcript pairs (sheet fragment : what whisper wrote), e.g. Elan Morin
# Tedronai: singer sings the fragments correctly, whisper spells English.
# Diagnostic only: gates still read plain lyric_recall above.
HOMOPHONES = {
    "elan": ["ee", "lan"], "moore": ["more"], "more": ["more"],
    "ted": ["ted"], "rogney": ["ro", "nye"], "rognay": ["ro", "nye"],
    "colamdon": ["koll", "am", "dahn"], "collam": ["koll", "am"],
    "lose": ["looze"], "loose": ["looze"], "shrin": ["theh", "rin"],
    "shrine": ["theh", "rin"], "wren": ["rin"], "par": ["pah", "ran"],
    "decent": ["dee", "sen"], "matt": ["mat"], "ran": ["rand"],
    "brand": ["rand"], "parent": ["perrin"], "inn": ["in"],
    "ice": ["eyes"], "die": ["dye"], "dye": ["dye"],
    "sigh": ["sigh"], "side": ["sigh"], "sided": ["sigh"],
    "loyal": ["loy", "al"], "eagle": ["ee", "gul"],
    "tower": ["tow"], "saddle": ["sah", "dul"],
}


def lyric_recall_alias(hyp: str, sheet: str) -> tuple[float, list[str]]:
    """(alias recall, recovered words): plain recall plus homophone vindication.

    The heard set is expanded with joined bigrams ("dragon mount" vindicates
    sheet word "dragonmount") and HOMOPHONES before the set-recall. Returns
    the recovered sheet words so reviewers can audit each vindication.
    """
    exp = norm_words(strip_sections(sheet))
    if not exp:
        return 0.0, []
    heard = norm_words(hyp)
    hset = set(heard)
    aliased = set(hset) | {heard[i] + heard[i + 1] for i in range(len(heard) - 1)}
    for w in heard:
        for frag in HOMOPHONES.get(w, ()):
            aliased.add(frag)
    recovered = sorted({w for w in set(exp) if w not in hset and w in aliased})
    return sum(w in aliased for w in exp) / len(exp), recovered
...
def score_ladder(lad: Ladder, meas: dict[str, dict], pool: list[np.ndarray],
    ...
    for s in scales:
        ...
        ch[f"{s:+g}"] = {
            ...
            "lyric_recall_alias": round(lyric_recall_alias(m.get("text", ""), lad.lyrics)[0], 4),
            "recall_recovered": lyric_recall_alias(m.get("text", ""), lad.lyrics)[1],
            "text": m.get("text", ""),
            "proj": proj,
            "proj_cos": cos(d, vhat),
...
def cmd_score(args) -> int:
    ...
    for folder in folders:
        ...
        out = folder / "lm_scores_alias.json"
...
def print_row(res: dict) -> None:
    ...
    print(f"{res['name']:<28s} {res['verdict']:<11s} "
          ...
          f"recA+={_fmt(p1.get('lyric_recall_alias'), '4.2f')} "
...
def write_board(results: list[dict], out: Path) -> None:
    ...
    for r in results:
        ...
        rows.append({
            ...
            "recall_alias_up": p1.get("lyric_recall_alias"),
...
def cmd_selftest(_args=None) -> int:
    ...
    # Alias layer (review copy only): homophone vindication must recover the
    # Elan Morin Tedronai fragments while leaving a true garble at 0, and
    # must never change the frozen plain recall the gates read.
    sheet = "[verse] Ee-lan More-in Ted-ro-nye sat at the Koll-am Dahn"
    heard = "Elan Moore and Ted Rognay sat at the Colamdon"
    plain = lyric_recall(heard, sheet)
    aliased, recovered = lyric_recall_alias(heard, sheet)
    assert plain < aliased, (plain, aliased)
    for frag in ("ee", "lan", "more", "koll", "am", "dahn"):
        assert frag in recovered, (frag, recovered)
    assert lyric_recall_alias("", sheet) == (0.0, [])
    assert lyric_recall_alias(heard, "") == (0.0, [])
    # compound vindication: "Dragon Mount" heard, "Dragonmount" on sheet
    a2, r2 = lyric_recall_alias("snow on Dragon Mount", "snow on Dragonmount")
    assert a2 == 1.0 and r2 == ["dragonmount"], (a2, r2)
    assert healthy["channels"]["+1"]["lyric_recall_alias"] >= \
        healthy["channels"]["+1"]["lyric_recall"]
    assert garble["channels"]["+1"]["lyric_recall_alias"] == 0.0
    assert garble["gates_fired"] == ["U2_lyric_hold"], garble["gates_fired"]
    ...
    print("lm_score selftest OK (garble, silence, song-replace, no-op[diagnostic], "
          ...
          "healthy uni + bipolar, alias-vindication)")
    return 0
