#!/usr/bin/env bash
# v20: leftover-gated real poles (PR #31)
# --lm_target faithful_sub_e_if_unused --pole_mode hidden
# Subtract leftover ê only when |ê̂_⊥ · â| < 0.50; else raw poles.
# Names are <axis>-lm-v20. Samples at eval/listen/v20/<axis>-lm-v20/.
# After each train: sidecar metrics + check_slider_gate.py, then
# generate_listen at scales -2,-1,0,1,2 (plus REFs).
#
#   ./scripts/train_v20_lm.sh 0 gender energy tempo distortion rapslow
#   ./scripts/train_v20_lm.sh 1 live breath rhyme triphop
set -uo pipefail
...
train_lm_v20() {
  ...
  mkdir -p "$save"
  wait_vram "$gpu"
  echo "[gpu${gpu}] TRAIN ${name} lm_target=faithful_sub_e_if_unused pole_mode=hidden endreg=1.0"
...
assert_v20_log() {
  ...
  if ! grep -q 'faithful_sub_e_if_unused: subtract leftover' "$log"; then
    echo "[v20] FAIL ${name}: expected leftover-gate teacher banner" >&2
    tail -40 "$log" || true
    return 1
  fi
  if ! grep -q 'pole_mode=hidden' "$log"; then
    echo "[v20] FAIL ${name}: expected pole_mode=hidden" >&2
    tail -40 "$log" || true
    return 1
  fi
  echo "[v20] leftover-gate pole_mode=hidden ok ${name}"
}

append_metrics() {
  local name="$1" sidecar="$2" gate_status="$3"
  local metrics="eval/listen/v20/metrics.tsv"
  mkdir -p "eval/listen/v20"
  if [ ! -f "$metrics" ]; then
    printf 'name\tlm_target\tpole_mode\tsteps\tcos_pos\tcos_neg\tcollapse\tedrift_p\tedrift_n\tloss\tgate\n' > "$metrics"
  fi
...
with open(out, "a") as fh:
    fh.write(row + "\n")
print(
    f"[v20] metrics {name} lm_target={d.get('lm_target')} "
...
run_one() {
  local gpu="$1" axis="$2"
  local name="${axis}-lm-v20"
  local prompts out logs sidecar
  prompts="$(prompts_for "$axis")" || {
    echo "[v20] no v4 pair file for $axis" >&2
    return 1
  }
  out="eval/listen/v20/${name}"
  logs="eval/listen/v20/logs"
  sidecar="models/${name}/${name}_last.json"
  mkdir -p "$logs" "$out" "models/${name}"
  echo "[v20] flags ${name} GPU${gpu} recipe=faithful_sub_e_if_unused pole_mode=hidden endreg=1.0"
  echo "  $PY -u conceptmod/textsliders/train_lm_slider_music3.py \\"
  echo "    --name ${name} --lm_target faithful_sub_e_if_unused --pole_mode hidden \\"
  ...
  if [ -f "models/${name}/${name}_last.safetensors" ]; then
    echo "[v20] SKIP train ${name} (weights exist)"
  else
    echo "[v20] ${name} GPU${gpu} prompts=$prompts lm_target=faithful_sub_e_if_unused"
    if ! train_lm_v20 "$gpu" "$name" "$prompts"; then
      echo "[v20] train failed ${name}"
      return 1
    fi
    if ! assert_v20_log "models/${name}/${name}_train.log" "$name"; then
      return 1
    fi
  fi
  local gate="FAIL"
  ...
  if CUDA_VISIBLE_DEVICES="$gpu" "$PY" scripts/check_slider_gate.py \
    ...
    echo "[v20] GATE OK ${name}"
    gate="OK"
  else
    echo "[v20] GATE FAIL ${name} (see $logs/${name}-gate.log) — still sampling"
  ...
  if ! sample_lm "$gpu" "$name" "$prompts" "$out"; then
    echo "[v20] sample failed ${name}"
    return 1
  fi
  echo "[v20] done ${name} -> $out"
}

if [ "$#" -lt 2 ]; then
  echo "usage: $0 <gpu> <axis> [axis...]" >&2
  echo "  catalog: $CATALOG" >&2
  echo "  recipe: --lm_target faithful_sub_e_if_unused --pole_mode hidden --endreg_weight 1.0" >&2
  exit 2
fi
GPU="$1"
shift
